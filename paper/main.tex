...
We present \textbf{Proof of Human Intent (PoHI)}, a protocol that combines zero-knowledge proofs of personhood (e.g., World ID), decentralized identifiers (DIDs), verifiable credentials (VCs), and transparency logs (SCITT) to create tamper-evident, machine-verifiable records of human approval.
...
\begin{tabular}{lccc}
\toprule
\textbf{System} & \textbf{Human} & \textbf{Crypto} & \textbf{Sybil} \\
...
Existing supply chain security frameworks (SLSA, Sigstore, SCITT) provide cryptographic binding and artifact integrity but do not verify that a \textit{human} performed the action. Proof-of-personhood systems (BrightID, Gitcoin Passport) verify human uniqueness but lack cryptographic binding to specific software artifacts. PoHI uniquely combines both properties, providing verifiable human approval with strong cryptographic guarantees.
...
\textbf{Theorem 1 (Unforgeability).} PoHI attestations are unforgeable under the soundness of the World ID ZK-SNARK system and collision resistance of SHA-256.
...
\bibitem{semgrep}
r2c, Inc., ``Semgrep: Lightweight Static Analysis for Security,'' 2024. [Online]. Available: https://semgrep.dev
