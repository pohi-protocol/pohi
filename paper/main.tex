...
We measure the latency of key operations in the PoHI workflow:

\begin{table}[h]
\centering
\caption{Operation Latency (measured on World Chain Sepolia)}
\begin{tabular}{lr}
\toprule
\textbf{Operation} & \textbf{Latency} \\
\midrule
Attestation creation (off-chain) & $<$ 10 ms \\
Hash computation (SHA-256) & $<$ 1 ms \\
World ID verification & 2-5 seconds \\
On-chain recording & 3-10 seconds \\
Attestation query & $<$ 100 ms \\
\bottomrule
\end{tabular}
\end{table}

The primary latency bottleneck is the World ID verification process, which requires user interaction with the World App. This is inherent to the human-in-the-loop design and cannot be optimized without compromising security.
