...
% Layer 2: Authority
\node[layer, fill=green!20] (authority) at (3.5, 3) {Authority\\(DID/VC)};
\node[desc, below=0.1cm of authority] {Permissions\\Credentials};

% Layer 3: Attestation
\node[layer, fill=orange!20] (attestation) at (7, 3) {Attestation\\(SCITT)};
\node[desc, below=0.1cm of attestation] {Transparency Log\\Hash Records};

% Layer 4: Integration (spans bottom)
\node[layer, fill=purple!20, minimum width=9cm] (integration) at (3.5, 0.5) {Integration Layer (GitHub Actions, Webhooks, Status API)};
...
\draw[arrow] (authority) -- (3.5, 1.3);
\draw[arrow] (attestation) -- (7, 1.3);

% External actors
\node[draw, ellipse, fill=gray!10, font=\footnotesize] (human) at (-2.5, 3) {Human};
\node[draw, ellipse, fill=gray!10, font=\footnotesize] (git) at (-2.5, 0.5) {Git/CI};
...
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners, minimum width=1.6cm, minimum height=0.7cm, align=center, font=\tiny},
...
% Labels
\node[font=\tiny, above=0.05cm of action] {1};
\node[font=\tiny, above=0.05cm of qr] {2};
\node[font=\tiny, above=0.05cm of scan] {3};
\node[font=\tiny, above=0.05cm of verify] {4};
\node[font=\tiny, above=0.05cm of attest] {5};
\node[font=\tiny, above=0.05cm of merge] {6};
