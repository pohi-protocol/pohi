<!--
ResearchHub preprint — paste-ready body.

The ResearchHub "Preprint" editor has fixed top-level sections
(Title / Abstract / Introduction / Methods / Results / Discussion).
Each block below is marked "PASTE INTO: <section>". Copy the content under each
marker into the matching editor section. Subheadings (##) can be added inside a
section with the editor's heading tool.
...
**Comparison.** Existing supply chain frameworks (SLSA, Sigstore, SCITT) provide artifact binding but no human verification or Sybil resistance. PoP systems (BrightID, Gitcoin Passport) verify human uniqueness but lack binding to specific artifacts. PoHI uniquely combines both: cryptographic proof of human identity bound to specific software artifacts, with strong Sybil resistance.
...
- *Replay resistance* — a valid attestation for commit c₁ cannot be used to verify approval for a different commit c₂ ≠ c₁.
...
**Theorem 2 (Replay resistance).** Under SHA-256 collision resistance. *Proof sketch:* replaying an attestation for c₁ on c₂ requires H(repo || c₁) = H(repo || c₂) with c₁ ≠ c₂ — a SHA-256 collision, infeasible under standard assumptions. ∎
...
End-to-end latency of the approval verification flow (reference implementation):
