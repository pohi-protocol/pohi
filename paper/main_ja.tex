...
図\ref{fig:architecture}は、レイヤーアーキテクチャとコンポーネント間のデータフローを示す。
...
\end{tikzpicture}
\caption{PoHI 4層アーキテクチャ。人間の承認はアイデンティティ検証（レイヤー1）、オプションの権限チェック（レイヤー2）、証明記録（レイヤー3）を経由し、すべて統合レイヤー（レイヤー4）を介して開発者ワークフローに接続される。}
...
図\ref{fig:flow}は、プルリクエスト作成からマージ有効化までの完全な検証フローを示す。
...
\end{tikzpicture}
\caption{検証フロー。(1) PRでGitHub Actionがトリガー、(2) QRコードコメントを投稿、(3) 人間がWorld Appでスキャン、(4) World ID APIでZK証明を検証、(5) 証明を作成・保存、(6) PRステータスを更新しマージを有効化。}
\label{fig:flow}
\end{figure}
