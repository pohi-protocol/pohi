...
export function canonicalizeAttestation(attestation: HumanApprovalAttestation): string {
  ...
  for (const field of CANONICAL_FIELD_ORDER.attestation) {
    ...
    if (value !== undefined) {
      if (field === 'subject') {
        canonical[field] = canonicalizeObject(
          value as Record<string, unknown>,
          CANONICAL_FIELD_ORDER.subject
        )
      } else if (field === 'human_proof') {
        canonical[field] = canonicalizeObject(
          value as Record<string, unknown>,
          CANONICAL_FIELD_ORDER.human_proof
        )
      } else {
...
export function validateAttestation(attestation: HumanApprovalAttestation): {
  ...
  if (
    attestation.subject?.action &&
    !knownActions.includes(attestation.subject.action as (typeof knownActions)[number])
  ) {
  ...
  if (
    attestation.human_proof?.method &&
    !knownProviders.includes(attestation.human_proof.method as (typeof knownProviders)[number])
  ) {
 ...
 */
export function verificationLevelToNumber(
  level: WorldIDVerificationLevel | string | undefined
): number {
